...
public class JobWorkerTests
{
    ...
    public async Task Start_VariousResults_Ok()
    {
        ...
        {
            Assert.All(group, i => Assert.Equal(JobResult.Unknown, GetExpectedResult(i.Item1.Key)));
        }
    ...
    {
        private readonly Func<TestJob, JobResult> _resultFn = resultFn ?? (_ => JobResult.Completed);
        ...
        public async Task<JobResult> HandleAsync(TestJob job, CancellationToken ct = default)
        {
            Jobs.Add(job);
            await Task.Delay(TimeSpan.FromSeconds(1), ct);
            return _resultFn(job);
        }
    }
